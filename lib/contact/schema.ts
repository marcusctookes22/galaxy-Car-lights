...
function validateAddOn(value: z.infer<typeof detailsSchema>, ctx: z.RefinementCtx) {
...
// Browser review and server delivery use the same detail rules. Delivery still
// independently requires consent, a submission ID and server-verified CAPTCHA.
export const contactDetailsSchema = detailsSchema.superRefine(validateAddOn);
export const contactSchema = detailsSchema.extend({
  submissionId: z.uuid(),
  consent: z.literal(true, { error: "Please agree to being contacted about your build enquiry." }),
  turnstileToken: z.string().min(1, "Complete the security check.").max(2048),
}).superRefine(validateAddOn);
