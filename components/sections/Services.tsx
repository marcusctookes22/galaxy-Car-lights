        ...
        <div className="border-t border-white/10">
          {services.map((service) => (
            <a
              ...
              className="group relative grid min-h-40 grid-cols-[64px_1.05fr_1fr_32px] items-center gap-x-8 gap-y-3 border-b border-white/10 py-8 transition-colors duration-300 hover:bg-white/[.025] focus-visible:outline-2 focus-visible:outline-offset-4 focus-visible:outline-[var(--gold)] max-lg:grid-cols-[44px_1fr_32px] max-sm:min-h-0 max-sm:grid-cols-[26px_1fr_24px] max-sm:gap-x-4 max-sm:gap-y-2 max-sm:py-6"
            >
              ...
              <p className="relative max-w-[420px] text-sm leading-7 text-[#a9a9a6] max-lg:col-start-2 max-lg:row-start-2">
                <span className="max-sm:hidden">{service.description}</span>
                <span className="sm:hidden">{service.mobileDescription}</span>
              </p>
